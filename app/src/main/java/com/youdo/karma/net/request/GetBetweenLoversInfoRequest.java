 ...
 */
public class GetBetweenLoversInfoRequest extends ResultPostExecute<List<BetweenLovers>> {
    public void request(){
        Call<ResponseBody> call = AppManager.getUserService()
                .getBetweenLoversInfo(AppManager.getClientUser().sessionId);
    ...
    private void parseJson(String json){
        try {
            ...
            Type listType = new TypeToken<ArrayList<BetweenLovers>>() {
            }.getType();
            Gson gson = new Gson();
            ArrayList<BetweenLovers> betweenLoverses = gson.fromJson(result, listType);
            onPostExecute(betweenLoverses);
        } catch (Exception e) {
            onErrorExecute(CSApplication.getInstance().getResources()
