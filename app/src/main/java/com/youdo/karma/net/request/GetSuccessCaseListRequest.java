 ...
 */
public class GetSuccessCaseListRequest extends ResultPostExecute<List<SuccessCase>> {
    public void request(){
        Call<ResponseBody> call = AppManager.getUserService()
                .getSuccessCaseList(AppManager.getClientUser().sessionId);
    ...
    private void parseJson(String json){
        try {
            ...
            Type listType = new TypeToken<ArrayList<SuccessCase>>() {
            }.getType();
            Gson gson = new Gson();
            ArrayList<SuccessCase> successCases = gson.fromJson(result, listType);
            onPostExecute(successCases);
        } catch (Exception e) {
            onErrorExecute(CSApplication.getInstance().getResources()
